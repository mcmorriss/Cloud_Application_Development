...
/*
function stringifyExample(idValue, nameValue, typeValue, lengthValue, protocolVal, hostVal, baseVal, boatLoads) {
...
router.get('/:boat_id', function(req, res) {
    ...
    .then( (boat) => {
        if (boat[0] == null) {
            ...
            res.status(200).type('json').send(stringifyExample(req.params.boat_id, boat[0].name, boat[0].type, boat[0].length, boat[0].loads, req.protocol + '://' + req.get("host") + req.baseUrl + '/' + req.params.boat_id))
...
router.post('/', function(req, res){
    if (req.body.name == null || req.body.type == null || req.body.length == null) {
        ...
        .then( key => {
            ...
            data.then(boatData => {
                res.status(201).send(stringifyExample(key.id, boatData[0].name, boatData[0].type, boatData[0].length, boatData[0].loads, req.protocol + '://' + req.get("host") + req.baseUrl + '/' + key.id))
            })
